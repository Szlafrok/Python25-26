W związku ze zmianą zasad aktywności Gigantów, od 03.12 obowiązuje nowy regulamin:

**Uwaga!** Zadania domowe wysyłają rodzice na przełomie miesiąca, podając jako odbiorców:

- sekretariat@giganciprogramowania.edu.pl
- kamiennygigant@gmail.com
  Dokładny termin wysłania zadań z danej lekcji jest zamieszczony w danym pliku z zadaniami do lekcji.

Alternatywnie zadania w mniejszych zestawach można podawać do sprawdzenia na koniec zajęć.

# Zasady aktywności 🥇 🙋‍♂️ ✨🎉

- Obecność i aktywność na zajęciach przeliczają się na punkty w Panelu Ucznia, które możemy wymieniać na nagrody.
- Za obecność na zajęciach uzyskujemy 10 punktów
- Za aktywność na zajęciach uzyskujemy od 1 do 5 gwiazdek. Każda gwiazdka przelicza się na 10 punktów.

Podstawowe gwiazdki: ⭐
Praca na lekcji i wykonywanie zadań: ❌ lub ⭐
Aktywność na zajęciach: ❌ lub ⭐ lub ⭐⭐, zależnie od liczby plusów. Jeśli zdobędziemy mniej niż 2 gwiazdki, część plusów przenosi się na następne zajęcia.
Rozwiązanie zadań dodatkowych: ❌ lub ⭐. Gwiazdkę zdobywamy po rozwiązaniu określonej części zadań. Na niektórych lekcjach, jeśli rozwiążemy ich więcej, możemy zarobić dodatkowe plusy do tej lub następnej lekcji!

### Praca ze wsparciem 🤖

Generalnie korzystanie z pomocy Czatu GPT oraz podobnych modeli AI jest dopuszczalne. To nie jest egzamin tylko zestaw ćwiczeń dla Was do szlifowania swoich umiejętności. Jednakże, żeby punkty były zdobywane uczciwie, musicie **rozumieć** rozwiązania skonstruowane z pomocą (lub przez) sztuczną inteligencję. Przy odsyłaniu poprawionych rozwiązań będę czasami prosił o krótkie wyjaśnienie rozwiązania któregoś z zadań i upewniał się, że wszystko tu rozumiemy - niekoniecznie w celu sprawdzenia Waszej wiedzy, często również z samego zainteresowania wykorzystaną metodą, chcąc rozwiać ewentualnie niejasności. Jest to również droga do gromadzenia plusów na kolejne zajęcia 😉
Również bardzo zachęcam do dopytywania z własnej inicjatywy! Jeśli np. napisaliście jakiś fragment zadania lub projektu z czatem GPT i nie rozumiecie go, możecie śmiało dopytywać o wyjaśnienia.
Uwaga - osoba, która wykona zadanie z pomocą Czatu GPT, a która zatai swoje niezrozumienie tematu, otrzyma 0 pkt za dane zadanie lub całą grupę zadań.
